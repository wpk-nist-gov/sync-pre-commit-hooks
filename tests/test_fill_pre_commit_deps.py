...
@pytest.mark.parametrize(
    ("options", "pre_commit_str", "python_version_file_str", "expected", "code"),
    [
        pytest.param(
            ["--hook=mypy", "--no-project-dependencies", "--python-version=3.12"],
            dedent("""\
repos:
  - repo: https://github.com/pre-commit/mirrors-mypy
    rev: v1.19.0
  ...
  - repo: https://github.com/pre-commit/mirrors-mypy
    ...
    hooks:
      - id: mypy
        ...
        pytest.param(
            ["--hook=mypy", "--python-version-file=.python-version"],
            dedent("""\
repos:
  - repo: https://github.com/pre-commit/mirrors-mypy
    rev: v1.19.0
...
def test_main_python_version(
    ...
    options: Sequence[str],
    pre_commit_str: str,
    python_version_file_str: str | None,
    expected: str,
    code: int,
) -> None:
    pre_commit_config = create_config_file(example_path, pre_commit_str)
    _ = create_config_file(example_path, example_pyproject, name="pyproject.toml")

    if python_version_file_str is not None:
        _ = create_config_file(
            example_path, python_version_file_str, name=".python-version"
        )

    assert fill_deps.main(options) == code
