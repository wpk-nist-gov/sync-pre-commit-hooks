...
This hook synchronizes dependency minimum versions with those in a locked
requirements file. This is useful to keep automation tools like dependabot
happy. For example, if you have a requirement file `requirements.in` which gets
locked to `requirements.txt`, and `requirements.in` is autogenerated from
`pyproject.toml` using a tool like
[pyproject2conda](https://github.com/usnistgov/pyproject2conda), dependabot will
update `requirements.in` without updating `pyproject.toml`. This hooks fixes
that by syncing the specified minimum version in `pyproject.toml` with those in
a locked `requirements.txt` file. This hook will also update
[PEP 723](https://peps.python.org/pep-0723/) sections of python scripts.
...
scripts to locked requirement file.

positional arguments:
  paths                 pyproject.toml/script files to process
...
options:
  ...
  --exclude EXCLUDE     Packages to exclude. Default is to consider all packages in
                        requirements file. Specifying ``--exclude`` will ignore those
